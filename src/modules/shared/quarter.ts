...
export function quarterStartForDate(date: Date, fiscalYearStartMonth = 7) {
  const month = date.getUTCMonth() + 1;
  const fiscalYearStart = month >= fiscalYearStartMonth ? date.getUTCFullYear() : date.getUTCFullYear() - 1;
  const offset = (month - fiscalYearStartMonth + 12) % 12;
  const quarterOffset = Math.floor(offset / 3) * 3;
  return new Date(Date.UTC(fiscalYearStart, fiscalYearStartMonth - 1 + quarterOffset, 1));
}

export function buildQuarterFromDates(startDate: Date, endDate: Date, locked = false, fiscalYearStartMonth = 7): ReportingQuarter {
  return {
    id: toIsoDate(startDate),
...
export function quarterDateRange(quarter: ReportingQuarter) {
  const start = parseIsoDate(quarter.startDate);
  const inclusiveEnd = parseIsoDate(quarter.endDate);
  if (!start || !inclusiveEnd) {
    throw new Error("Quarter configuration is invalid.");
  }
...
export function buildQuarterTimeline(
  rangeStart: Date,
  rangeEnd: Date,
  fiscalYearStartMonth = 7,
  locked = false
): ReportingQuarter[] {
  const quarters: ReportingQuarter[] = [];
  let cursor = quarterStartForDate(rangeStart, fiscalYearStartMonth);
  const finalStart = quarterStartForDate(rangeEnd, fiscalYearStartMonth);

  while (cursor <= finalStart) {
    const { startDate, endDate } = nextQuarterDates(cursor);
    quarters.push(buildQuarterFromDates(startDate, endDate, locked, fiscalYearStartMonth));
